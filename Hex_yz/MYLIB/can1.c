...
short Real_Current_Value[6] = {0};
short Real_Velocity_Value[6] = {0};
long Real_Position_Value[6] = {0};
        ...
        if((rx_message.IDE == CAN_Id_Standard)&&(rx_message.IDE == CAN_RTR_Data)&&(rx_message.DLC == 8)) //标准帧、数据帧、数据长度为8
        {
            ...
            else if(rx_message.StdId == 0x4B)
            {
                ...
                Real_Position_Value[3] = ((rx_message.Data[4]<<24)|(rx_message.Data[5]<<16)|(rx_message.Data[6]<<8)|(rx_message.Data[7]));
						...
						else if(rx_message.StdId == 0x5B)
            {
                Real_Current_Value[4] = (rx_message.Data[0]<<8)|(rx_message.Data[1]);
                Real_Velocity_Value[4] = (rx_message.Data[2]<<8)|(rx_message.Data[3]);
                Real_Position_Value[4] = ((rx_message.Data[4]<<24)|(rx_message.Data[5]<<16)|(rx_message.Data[6]<<8)|(rx_message.Data[7]));
            }
						else if(rx_message.StdId == 0x6B)
            {
                Real_Current_Value[5] = (rx_message.Data[0]<<8)|(rx_message.Data[1]);
                Real_Velocity_Value[5] = (rx_message.Data[2]<<8)|(rx_message.Data[3]);
                Real_Position_Value[5] = ((rx_message.Data[4]<<24)|(rx_message.Data[5]<<16)|(rx_message.Data[6]<<8)|(rx_message.Data[7]));
            }
